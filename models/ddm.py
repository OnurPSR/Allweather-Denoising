...
def noise_estimation_loss(model, x0, t, e, b, psnr_weight=0.0, ssim_weight=0.0):
    ...
    x0_pred = (x - output * (1.0 - a).sqrt()) / denom
...
class DenoisingDiffusion(object):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss_dict = noise_estimation_loss(
                    ...
                    e,
                    b,
                    psnr_weight=getattr(self.config.training, 'psnr_weight', 0.0),
                    ssim_weight=getattr(self.config.training, 'ssim_weight', 0.0)
                )
                loss = loss_dict['total_loss']

                if self.step % 10 == 0:
                    print(
                        f"step: {self.step}, total_loss: {loss.item():.6f}, "
    ...
    def sample_validation_patches(self, val_loader, step):
        ...
        with torch.no_grad():
            ...
            val_psnr = calculate_psnr_torch(x, x_gt)
            val_ssim = calculate_ssim_torch(x, x_gt)
